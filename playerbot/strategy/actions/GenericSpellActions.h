...
namespace ai
{
    ...
    {
    public:
        GreaterBuffOnPartyAction(PlayerbotAI* ai, std::string spell, bool ignoreTanks = false, std::string lowerSpell = "") :
            CastBuffSpellAction(ai, spell), PartyMemberActionNameSupport(spell), ignoreTanks(ignoreTanks), lowerSpell(lowerSpell) {}
    ...
    protected:
        ...
        {
            std::string qualifier = GetSpellName();
            if (!lowerSpell.empty())
                qualifier += "," + lowerSpell;

            return qualifier + "-" + (ignoreTanks ? "1" : "0");
        }
