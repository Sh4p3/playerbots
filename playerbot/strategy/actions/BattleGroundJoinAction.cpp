...
bool BGJoinAction::gatherArenaTeam(ArenaType type)
{
    ...
    for (ArenaTeam::MemberList::iterator itr = arenateam->GetMembers().begin(); itr != arenateam->GetMembers().end(); ++itr)
    {
        ...
        if (member)
        {
            ...
            if (member->IsBeingTeleported() || member->GetMapId() != bot->GetMapId())
            {
...
bool BGJoinAction::shouldJoinBg(BattleGroundQueueTypeId queueTypeId, BattleGroundBracketId bracketId)
{
    ...
    ArenaTeam* captainArenaTeam = nullptr;
    if (type != ARENA_TYPE_NONE)
    {
        ...
        for (uint8 i = 0; i < MAX_ARENA_SLOT; ++i)
        {
            ...
            ArenaTeam* team = sObjectMgr.GetArenaTeamById(a_id);
            if (!team)
                continue;
    ...
    if (isArena)
    {
        ...
        if (!hasPlayers && captainArenaTeam)
            isRated = true;

        // do not queue skirmish without real players
        if (!hasPlayers && !(captainArenaTeam || isRated))
            return false;
...
bool BGStatusCheckAction::Execute(Event& event)
{
    ...
    if (hasInvite)
    {
        static constexpr char invitedPollValue[] = "bg invited status poll";
        int32 now = int32(time(nullptr));
        int32 lastPoll = AI_VALUE2_EXISTS(int32, "manual int", invitedPollValue, 0);
        if (lastPoll > 0 && (now - lastPoll) < 2)
            return false;
        SET_AI_VALUE2(int32, "manual int", invitedPollValue, now);
    }
...
{
    return bot->InBattleGroundQueue() && !bot->InBattleGround();
}
